...
calculateCraftAmounts = function() {
  ...
  for (var i = 0; i < resources.length; i++) {
    ...
    for (var j = 0; j < prices.length; j++) {
      ...
      if (checkVal > prices[j].val) amount = Math.max(amount, Math.ceil(checkVal/prices[j].val))
    }
...
function buildingCalculator() {
  ...
  buildings.sort(function(a, b){return a.label.localeCompare(b.label)});
...
function calculateBuildingPrice() {
  ...
  for (var i = 0; i < bld.prices.length; i++) {
    ...
    if (bldName[0] == 'space' && (bld.prices[i].name == "oil" || bld.prices[i].name == "rocket")) {
      ...
      if (calcPrice(bld.prices[i].val, priceRatio, j) > res.maxValue) {
        resLimit = j;
        break;
      }
    }
  ...
  if (number > 0) {
    ...
    for (var i = 0; i < bld.prices.length; i++) {
      ...
      if (bldName[0] == 'space' && (bld.prices[i].name == "oil" || bld.prices[i].name == "rocket"))
      {
        ...
        finalPrice = calcPrice(bld.prices[i].val, priceRatio, number - 1);
      var res = gamePage.resPool.get(bld.prices[i].name);
    ...
    if (bld.val < number) {
      ...
      for (var i = 0; i < bld.prices.length; i++) {
        ...
        if (bldName[0] == 'space' && (bld.prices[i].name == "oil" || bld.prices[i].name == "rocket")) {
          ...
          price += calcPrice(bld.prices[i].val, priceRatio, j);
        }
        var res = gamePage.resPool.get(bld.prices[i].name);
